...
io.on("connection", (socket) => {
  ...
  // Join room
  socket.on("joinRoom", async ({user,other})=>{
    // Check if room exists
    await db.read();
    const room = db.data.chatRooms.find((r) => r.users.includes(user.id) && r.users.includes(other.id));
  ...
  // Send message
  socket.on("messageSent", async ({ room, message }) => {
    await db.read();
